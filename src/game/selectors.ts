...
export function getItemsInCurrentRoom(
  ...
  return state.world.items.filter((it) => it.location === state.player.roomId);
}
...
export function describeCurrentEffects(state: GameState): string {
  const statusEffects = state.player.statuses ?? [];

  let effectsMsg = "";
  for (const statusEffect of statusEffects) {
    const effectId = statusEffect.id;
    const intensity = statusEffect.intensity;
    switch (effectId) {
      case "drunk":
